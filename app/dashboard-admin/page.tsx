...
export default function DashboardAdmin() {
  ...
  // State Personal Branding (Database Supabase)
  ...
  const fetchBranding = async () => {
    const { data, error } = await supabase.from('personal_branding').select('*').single();
    if (!error && data) {
      setBranding(data);
    }
  };
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert('Gagal upload foto. Pastikan bucket "branding-photos" sudah dibuat.');
    } finally {
      setUploading(false);
    }
  };
  ...
  const removePhoto = async () => {
    if (branding.photo_url) {
      // Optional: Delete from storage if needed
      // const path = branding.photo_url.split('/').pop();
      // await supabase.storage.from('branding-photos').remove([path || '']);
    }
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800 pb-6">
          <div>
            <h1 className="text-3xl font-extrabold text-teal-400 tracking-tight">Admin Control Panel</h1>
            <p className="text-sm text-slate-500 mt-1">Kelola portofolio & personal branding via Supabase.</p>
          </div>
              ...
              <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl sticky top-6">
                ...
                {branding.is_multi_language && (
                  <div className="mb-4 p-3 bg-purple-900/20 border border-purple-500/30 rounded-lg">
                    <p className="text-[10px] text-purple-300 font-bold">🌍 Mode Multi-Bahasa Aktif ({branding.secondary_lang.toUpperCase()})</p>
                  </div>
                )}
        ...
        {activeTab === 'branding' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl sticky top-6">
                <h2 className="text-sm font-bold text-purple-400 mb-6">Pengaturan Profil (Supabase)</h2>
                ...
                <div className="mb-6 flex flex-col items-center">
                  ...
                  <p className="text-[10px] text-slate-500 text-center">Disimpan permanen di Supabase Storage</p>
                </div>
                ...
                <div className="mb-6 space-y-2">
                  ...
                  <textarea 
                    ...
                    onBlur={() => saveBrandingToDb(branding)} // Auto-save on blur
                ...
                <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      ...
                      className={`w-11 h-6 rounded-full relative transition-colors ${branding.is_multi_language ? 'bg-purple-500' : 'bg-slate-700'}`}
                    >
              ...
              <div className="bg-gradient-to-br from-slate-900 to-purple-950/10 border border-purple-900/20 p-8 rounded-2xl min-h-[400px] flex flex-col items-center justify-center text-center space-y-6">
                ...
                {branding.is_multi_language && (
                  <div className="px-4 py-1.5 bg-purple-900/30 border border-purple-700/30 rounded-full text-xs text-purple-200 font-medium">
                    🌐 Available in {branding.secondary_lang.toUpperCase()}
                  </div>
                )}
              </div>
            </div>
